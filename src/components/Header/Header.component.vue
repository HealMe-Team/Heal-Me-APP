    ...
    <div class="header-content">
      ...
      <div class="profile-menu-container">
        ...
        <div v-if="showMenu" class="dropdown-menu">
          <ul>
            <li @click="navigateTo('/doctor-view')">Doctor</li>
            <li @click="navigateTo('/')">User</li>
          </ul>
        </div>
      </div>
...
.profile-menu-container {
  ...
  display: flex;
  align-items: center;
  margin-left: 10px; /* Ajusta este valor según sea necesario para el espacio a la derecha del logo */
}
...
.dropdown-menu {
  position: absolute;
  top: 50%;
  left: 60px; /* Ajusta la distancia del cuadro al lado derecho de la imagen */
  transform: translateY(-50%); /* Para centrar verticalmente el menú al lado de la imagen */
  background-color: white;
  border: 1px solid #ccc;
  border-radius: 5px;
  box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1);
  z-index: 100;
  min-width: 120px; /* Asegura que el menú tenga un ancho mínimo */
}
